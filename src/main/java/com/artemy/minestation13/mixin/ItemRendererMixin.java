...
public abstract class ItemRendererMixin {
    ...
    public BakedModel renderItem(BakedModel bakedModel, @Local(argsOnly = true) ItemStack stack, @Local(argsOnly = true) ModelTransformationMode renderMode) {
        if (stack.getItem() == ModItems.SPECTRE_STAFF && (renderMode == ModelTransformationMode.GUI || renderMode == ModelTransformationMode.GROUND || renderMode == ModelTransformationMode.FIXED)) {
            return getModels().getModelManager().getModel(ModelIdentifier.ofInventoryVariant(MineStation13.id("spectre_staff")));
        }
    ...
    public BakedModel getHeldItemModelMixin(BakedModel bakedModel, @Local(argsOnly = true) ItemStack stack) {
        if (stack.getItem() == ModItems.SPECTRE_STAFF) {
            return this.models.getModelManager().getModel(ModelIdentifier.ofInventoryVariant(MineStation13.id("spectre_staff_3d")));
        }
